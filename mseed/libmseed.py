...
Selected examples of Methods in mseed.libmseed
----------------------------------------------------
#For reading traces (channels)
from mseed.libmseed import LibMSEED
libmseed=LibMSEED()
data=libmseed.readMSTraces(filename) #filename is a valid miniseed full file path

#For miniseed file validation
libmseed.isMSEED(filename)

#For header reading without traces
header=libmseed.readMSHeader(filename)

#For start and end time quering
datetime=libmseed.getStartAndEndTime(filename)

Other methods need further testing

"""

from mseed.headers import MSFileParam, _PyFile_callback, clibmseed, \
...
try:
    from UTCDateTime.UTCDateTime import UTCDateTime #Separate UTCDateTime package is prefferable
except ImportError:
    from mseed.UTCDateTime import UTCDateTime #Embedded solution
...
class LibMSEED(object):
    ...
    def readMSTracesViaRecords(self, filename, reclen= -1, dataflag=1,
        ...
        if starttime or endtime:
            bytez = self._bytePosFromTime(filename, starttime=starttime,
                                          endtime=endtime)
            if bytez == '':
                del ms # for valgrind
                return ''
            ms.offset = bytez[0]
            end_byte = bytez[0] + bytez[1]
        else:
        ...
        while True:
            ...
            if header['samplecnt'] != 0: #Geosig files handling

                delta = HPTMODULUS / float(header['samprate'])
                header['endtime'] = int(header['starttime'] + delta * \
                                          (header['numsamples'] - 1))
    ...
    def readFileToTraceGroup(self, filename, reclen= -1, timetol= -1,
        ...
        if starttime and endtime:
            ...
            errcode = clibmseed.ms_readtraces_timewin(
                C.pointer(mstg), filename.encode('cp1251'), reclen, timetol, sampratetol,
    ...
    def readMSHeader(self, filename, reclen= -1, timetol= -1,
        ...
        for _ in range(mstg.contents.numtraces - 1):
        ...
        #cleaning empty traces. Geosig handling
        clear_header=[]
        [clear_header.append(h) for h in header if h[0]['samplecnt'] != 0]
        return clear_header
    ...
    def getFileformatInformation(self, filename):
        ...
        May give wrong results for Geosig
        """
    ...
    def getDataQualityFlagsCount(self, filename):
        ...
        for _i in range(info['number_of_records']):
            # Skip non data records.
    ...
    def getTimingQuality(self, filename, first_record=True,
        ...
        for _i in range(fileinfo['number_of_records']):
    ...
    def _ctypesArray2NumpyArray(self, buffer, buffer_elements, sampletype):
        ...
        numpy_array = np.empty(buffer_elements, dtype=sampletype.decode())
        ...
        C.memmove(datptr, buffer, buffer_elements * SAMPLESIZES[sampletype.decode()])
        return numpy_array
    ...
    def _populateMSTG(self, trace):
        ...
        sampletype = trace[0]['sampletype'].decode()
...
class _MSStruct(object):
    ...
    def __init__(self, filename, init_msrmsf=True):
        ...
        #TODO Cirillic problem. Try UTF-8 first
        self.file = filename.encode('cp1251') #2to3 Cirillic problem
...
class TestSuite(unittest.TestCase):
    def test_read_various_miniseed_files(self):
        '''Check for reading different miniseed files from different sources'''
        libmseed=LibMSEED()
        currentdir=os.path.dirname(__file__)
        testfilespath=os.path.join(currentdir,'resources','testfiles','*.*')
        list_of_testfiles=[name for name in glob.glob(testfilespath) if libmseed.isMSEED(name)]
        for filename in list_of_testfiles:
            data=libmseed.readMSTraces(filename)

##    def test_write_miniseedfile(self): #Failed on my Windows 8 machine
##        libmseed=LibMSEED()
##        #Read valid miniseed
##        testfilename='blockette008.mseed'
##        currentdir=os.path.dirname(__file__)
##        filename=os.path.join(currentdir,'resources','testfiles',testfilename)
##        data=libmseed.readMSTraces(filename)
##        #Trying to write a file
##        #writeMSTraces test
##        libmseed.writeMSTraces(data,os.path.join(currentdir,'resources','testfiles','wrotefile.msd'))
...
##path='D:/Work/PythonExp/mseed/resources/testfiles/*.*'
##list_of_testfiles=[ name for name in glob.glob(path) if libmseed.isMSEED(name)]
##
##for filename in list_of_testfiles:
##  #readMSTraces test
##    print(filename)
##    data=libmseed.readMSTraces(filename) #Most useful
##    print(data)



##    #filename="D:/Work/PythonExp/mseed/test.mseed"
##
##    #LibMSEED method tests
##    libmseed=LibMSEED()
##    #isMSEED test
##    isminiseed=libmseed.isMSEED(filename)
##    print('isMSEED method test')
##    print(filename + ' is a miniseed file: '+ str(isminiseed))
##    print('Test passed')
##    #readMSTracesViaRecords test
##    libmseed.readMSTracesViaRecords(filename)
##    print('readMSTracesViaRecords method test')
##    print('Test passed')
##    #readMSTraces test
##    data=libmseed.readMSTraces(filename) #Most useful
##    print('readMSTraces method test')
##
##    import matplotlib.pyplot as plt
##    plt.plot(range(0,data[0][1].__len__()),data[0][1],'o',range(0,data[1][1].__len__()),data[1][1],'s',range(0,data[2][1].__len__()),data[2][1],'o',alpha = 0.5)
##    plt.fill_between(range(0,data[0][1].__len__()),data[0][1],facecolor='blue', alpha = 0.5)
##
##    plt.grid(True)
##
##    plt.tight_layout()
...
##    print('Test passed')
##
##    #readFileToTraceGroup test
##    traceGroup=libmseed.readFileToTraceGroup(filename)
##    print('readFileToTraceGroup method test')
##    print('Test passed')
##
##    #getStartAndEndTime test
##    datetime=libmseed.getStartAndEndTime(filename)
##    print('getStartAndEndTime method test')
##    print('Test passed')
##
##    #readMSHeader test
##    header=libmseed.readMSHeader(filename)
##    print('readMSHeader method test')
##    print('Test passed')
##
##    #getFileformatInformation test
##    fileinfo=libmseed.getFileformatInformation(filename)
##    print('getFileformatInformation method test')
##    print('Test passed')
##
##    #getDataQualityFlagsCount test
##    dataquality=libmseed.getDataQualityFlagsCount(filename)
##    print('getDataQualityFlagsCount method test')
##    print('Test passed')
##
##    #getTimingQuality test
##    timingquality=libmseed.getTimingQuality(filename)
##    print('getTimingQuality method test')
##    print('Test passed')
##
##
####    #writeMSTraces test
####    libmseed.writeMSTraces(data,os.path.join(currentdir,'resources','testfiles','wrotefile.msd'))
####    print('writeMSTraces method test')
####    print('Test passed')
